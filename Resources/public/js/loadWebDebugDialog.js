function Domis86WebDebugDialogLoaderClass() {
    ...
    this.startInit = function () {
        var assets = document.getElementById('domis86_data_for_loadwebdebugdialog');
        yepnope({
            test: (typeof jQuery !== 'undefined'),
            nope: assets.getAttribute("data-jquery_url_1"),
            complete: function () {
    ...
    function loadLibs() {
        var dataContainer = jQuery('#domis86_data_for_loadwebdebugdialog');
        that.backendMode = ('on' === dataContainer.data('backend_mode'));
        that.assetsBasePath = dataContainer.data('assets_base_path');
    ...
    this.tryToInitDialog = function () {
        if (requirementsAreLoaded()) {
            var domis86WebDebugDialog = new Domis86WebDebugDialogClass(that.backendMode, that.assetsBasePath);
            domis86WebDebugDialog.prepare();
            return true;
        }

        consoleLog('Domis86WebDebugDialog requirements are not loaded yet...');
        setTimeout(function tryToInitDialogTimeoutCallback() {
            that.tryToInitDialog();
        }, 200);
        return false;
    };
    ...
    function requirementsAreLoaded() {
        ...
        if (!isDialogContainerLoaded) { return false; }
